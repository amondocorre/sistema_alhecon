...
if (@file_exists(dirname(__FILE__) . '/lang/eng.php')) {
  ...
  $pdf->Cell(0, 5, "MOVIMIENTO DE CAJA: $data->tipoMovimieno", 0, 1, 'C');
  // $pdf->Cell(0, 0, '------------------------------------------------', 0, 1, 'C');
  //$pdf->Cell(0, 0, '', 'T', 1, 'C');
  $pdf->SetFont('helvetica', 'B', 10);
  $pdf->Cell(23, 5, "Sucursal: ", $margen, 0, 'L');
  $pdf->SetFont('helvetica', '', 10);
  $pdf->MultiCell(48, 5, "$data->sucursal", $margen, 'L',false);
  $pdf->SetFont('helvetica', 'B', 10);
  $pdf->Cell(23, 5, "Usuario: ", $margen, 0, 'L');
  $x = $pdf->GetX();
  $y = $pdf->GetY();
  $pdf->SetXY($x, $y);
  $pdf->SetFont('helvetica', '', 10);
  ...
  $pdf->SetFont('helvetica', 'B', 10);
  $pdf->Cell(23, 5, "Monto: ", $margen, 0, 'L');
  $pdf->SetFont('helvetica', '', 10);
  $pdf->Cell(48, 5, "$data->monto", $margen, 1, 'L');
  $pdf->SetFont('helvetica', 'B', 10);
  $pdf->Cell(23, 5, "Fecha: ", $margen, 0, 'L');
  $pdf->SetFont('helvetica', '', 10);
  $pdf->Cell(48, 5, "$data->fecha", $margen, 1, 'L');
  $pdf->SetFont('helvetica', 'B', 10);
  $pdf->Cell(23, 5, "Hora: ", $margen, 0, 'L');
  $pdf->SetFont('helvetica', '', 10);
  $pdf->Cell(48, 5, "$data->hora", $margen, 1, 'L');  
  $pdf->SetFont('helvetica', 'B', 10);
  $pdf->Cell(23, 5, "Descripción:", $margen, 0, 'L');
  $x = $pdf->GetX();
  $y = $pdf->GetY();
  $pdf->SetXY($x, $y);
  $pdf->SetFont('helvetica', '', 10);
  $pdf->MultiCell(48, 5, "$data->descripcion", $margen, 'L', false);
  //$pdf->Cell(0, 0, '', 'T', 1, 'C');
  $pdf->Ln(8);
  $pdf->Cell(0, 3, "----------------------", 0, 1, 'C');
  $pdf->SetFont('helvetica', 'B', 10);
  $pdf->Cell(0, 5, $data->usuario, 0, 1, 'C');
